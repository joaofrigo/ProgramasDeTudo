...
def processar_centro(file_path, centro_nome):
    ...
    # Filtra apenas formandos positivos e agrupa por sexo
    df_resultado = df[df["FORMADOS"] > 0].groupby(["ANO", "SEXO"])['FORMADOS'].sum().reset_index()
...
# Definindo cores diferentes para os centros e sexos
cores = {
...
for i, sexo in enumerate(["M", "F"]):
    for j, centro in enumerate(centros):
        ...
        barras = [subset[subset["ANO"] == ano]["FORMADOS"].values[0] if not subset[subset["ANO"] == ano].empty else 0 for ano in anos]
...
plt.title('Número de Formandos por Ano e Sexo nos Centros de Ensino')
